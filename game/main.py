...
@app.route('/api/game/<kind>', methods=['POST'])
def create(kind):
  game_obj = game.create(kind)
  response = {'game_id': game_obj.key.urlsafe()}
  return json.dumps(response)

@app.route('/api/game/<game_id>', methods=['PUT'])
def connect(game_id):
  (token, player_id) = game.connect(ndb.Key(urlsafe=game_id))
...
@app.route('/api/game/<game_id>/<player_id>', methods=['POST'])
def event(game_id, player_id):
  event = request.get_json()
  game_key = ndb.Key(urlsafe=game_id)
  response = game.process_event(game_key, player_id, event)
  return json.dumps(response)
